        ...
        for(RegionDAO region: regions){
            ...
            for(Element row:rows){
                try{
                    ...
                    UniversityDAO temp = new UniversityDAO(universityHref, universityName, maxBudgetPlaces, allPlaces, applies, originals);
                    session.save(temp);
                }catch (Exception ex){
            ...
            for(int pageNumber=1; true; pageNumber++){
                try {
                    ...
                    for(Element row: rows){
                        ...
                        SpecialityDAO speciality = new SpecialityDAO(okr, appliesHref, direction, applies, allPlaces, maxBudgetPlaces,originals,subjects);
            ...
            for(int pageNumber=1; true; pageNumber++){
                try {
                    ...
                    for(Element row: rows){
                        ...
                        ApplyDAO apply = new ApplyDAO(place, pib, priority,documents,  bal,  atestat,  zno,  extraBaly);
